...
import { deletePost, editPost } from "@/helpers/postHandler";
// import { Post } from "./PostType"; // if you export Post type, else define locally
          ...
          <div className="bg-white rounded-2xl max-w-xl w-full max-h-[90vh] overflow-y-auto shadow-xl relative">
            ...
            <div className="flex overflow-x-auto gap-2 p-4 scrollbar-hide">
              {selectedPost.media_urls.map((url, i) => (
                <div
                  key={i}
                  className="flex-shrink-0 w-[250px] h-[400px] relative rounded-lg overflow-hidden shadow-md"
                >
                  {url.endsWith(".mp4") ? (
                    <video
                      src={url}
                    ...
                    <img
                      src={url}
                      alt={selectedPost.caption || `Media ${i + 1}`}
                      className="w-full h-full object-cover rounded-lg"
                    />
                  )}
                </div>
